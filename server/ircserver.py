...
class Channel(object):
    ...
    def add_member(self, client):
        ...
        timestamp = datetime.utcnow().strftime("%H:%M:%S") # %Y-%m-%d %H:%M:%S UTC
        print timestamp, client.nickname, 'joined', '('+client.host+':'+str(client.port)+')'
    ...
    def remove_client(self, client):
        ...
        timestamp = datetime.utcnow().strftime("%H:%M:%S") # %Y-%m-%d %H:%M:%S UTC
        print timestamp, client.nickname, '\tleft'
        if not self.members:
...
class Client(object):
    ...
    def __registration_handler(self, command, arguments):
        ...
        if self.nickname and self.user:
            self.reply("001 %s :irc server running" % self.nickname)
            self.reply("002 %s :host is %s" % (self.nickname, server.name))
            self.send_lusers()
            self.send_motd()
    ...
    def __command_handler(self, command, arguments):
        ...
        def nick_handler():
            ...
            if newnick == self.nickname:
                ...
                print timestamp, oldnickname, 'is now:', self.nickname
