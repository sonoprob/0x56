function handle_command(chnl, cmd, args)
    ...
    elseif cmd == "reboot" then
        ...
        node.restart()
    elseif cmd == "who" then send_message(chnl, "/names")    
    elseif cmd == "int" then print('hiD'); node.input([[print("hello blublube")]])   
    elseif cmd == "reload" then dofile('ircoverload.lua')
    elseif cmd == "heap" then send_message(chnl, node.heap() .. ' bytes remaining')
    elseif cmd == "mac" then send_message(chnl, wifi.sta.getmac())
    elseif cmd == "update" then Upgrader.update('test2.lua', 'http://78.47.95.86/test2.lua')
    end
